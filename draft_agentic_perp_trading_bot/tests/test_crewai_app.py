...
def test_signal_evaluation_rejects_review_supplied_omitted_stop_loss() -> None:
    message = _message()
    evaluation = _evaluation(message)
    tier = StrategyTier.INTERMEDIATE
    review = evaluation.reviews.reviews[tier]
    ...
    with pytest.raises(ValueError, match="supplied a stop-loss omitted by QWEN"):
        altered_evaluation.validate_for_message(message)
